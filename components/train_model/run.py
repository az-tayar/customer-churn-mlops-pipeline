...
TRAIN_FILE = 'train_data_metrics.json'
VAL_FILE = 'val_data_metrics.json'
...
def go(args):
    ...
    with open(f'{METRICS_DIR}/{TRAIN_FILE}', 'w') as f:
    ...
    with open(f"{VIS_DIR}/{CONF_MATRIX_FILE}", "wb") as f:
    ...
    os.makedirs(METRICS_DIR, exist_ok=True)
    with open(F'{METRICS_DIR}/{VAL_FILE}', 'w') as f:
